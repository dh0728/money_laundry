...
- 유형은 코드(0~8) + 명칭 매핑표를 이 문서에 고정 `[미정: 모델 출력 형식 확정 시 매핑표 기입]`.
...
### 워커→BE 점수 산출물 (거래 1건당)
현 W1 더미 형식 — **`[미정: run_114 실제 출력 형식 확정 시 이 표를 갱신]`**

| 필드 | 내용 |
|---|---|
| `tx_row` | 업로드 CSV 내 행 번호 (W2 DB 전환 시 거래 식별자로 대체) |
| `anomaly_score` | 점수 1 (임시명) `[미정]` |
| `type_score` | 점수 2 (임시명) `[미정]` |
| `type_class` | 유형 코드 0~8 |
| `rule_hits[]` | 적중 룰 식별자 (룰 기반은 10월 이후 — 그전까지 빈 배열) |
...
- 행: `{ "uploadId", "txRow", "anomalyScore", "typeScore", "typeClass", "ruleHits[]" }` (필드명은 층 2 확정에 연동)
...
1. 거래 점수 2종의 실제 의미·필드명 (run_114 출력 형식 — 사용자 확정 대기)
2. 유형 코드 0~8 ↔ 명칭 매핑표
