...
- 정책 `daily-seed-link-v2`: 씨앗 전후 24시간, 깊이2, 최대100거래, 창 내 계좌 활동100건 초과 시 확장 중지. 한 후보에 다른 후보의 씨앗 거래가 포함될 때 병합할 수 있다. 비씨앗 거래를 공유한다는 이유만으로 병합하지 않으며, 정상 예측·미채점 거래의 탐색과 포함은 유지한다. 병합 최대100거래/48시간 범위를 유지한다. 패턴 확률은 주석이며 탐색·병합 조건이 아니다. 과거 조회·관련 변경 기반 재검토 설계는 아직 이 정책에 적용하지 않았다.
