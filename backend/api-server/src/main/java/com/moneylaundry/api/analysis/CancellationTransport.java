...
public class CancellationTransport implements AnalysisRunService.CancelTransport {
  ...
  private String key(String payload, boolean output) {
    ...
    return (output ? "results/" : "requests/")
        + d.get("job_id").asLong()
        + "/"
        + d.get("model_kind").asString()
        + "/"
        + UUID.fromString(d.get("request_id").asString())
        + "/rounds/"
        + d.get("execution_round").asInt()
        + "/"
        + (output ? "cancel_ack.json" : "cancel.json");
  }
  ...
  private void same(String key, tools.jackson.databind.JsonNode document) {
    ...
    if (existing == null || !mapper.readTree(existing).equals(document))
      throw new IllegalStateException("CANCEL_IMMUTABLE_CONFLICT");
  }
  ...
  public String acknowledgement(UUID cancel, String payload) {
    ...
    if (!cancel.toString().equals(actual.get("cancel_id").asString()))
      throw new IllegalStateException("CANCEL_ACK_IDENTITY_MISMATCH");
    String status = actual.path("status").asString();
    if (!List.of("STOPPED", "ALREADY_FINISHED").contains(status))
