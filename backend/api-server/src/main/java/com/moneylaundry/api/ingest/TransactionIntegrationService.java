...
public class TransactionIntegrationService {
  ...
  public Outcome integrate(LocalDate businessDate, Instant cutoff, Set<Long> fixedUploadIds) {
    ...
    return tx.execute(status -> integrateLocked(businessDate, cutoff, Set.copyOf(fixedUploadIds)));
  }
  ...
  private void connect(long txId, ReportMatcher.Report report) {
    TransactionRow r = report.row();
    jdbc.update(
        "insert into transaction_reports values(?,?,?)",
        txId,
        report.id(),
        r.fromBank() == r.toBank()
            ? "INTERNAL"
  ...
  private boolean conflictsWithConfirmed(TransactionRow row, Map<List<String>, Boolean> cache) {
    return conflict(
            row.fromBank(),
            row.fromBankName(),
            row.fromAccount(),
