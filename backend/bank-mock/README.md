...
[API.md의 은행 수집 API](../api-server/API.md#11-은행-수집-api)에 따라 은행의 하루치 거래 CSV를 S3에 올린 뒤 보고 검수·저장 결과와 통합 상태를 확인한다. 은행 측은 데이터 공급 목업만 제공하며 은행 직원 웹·로그인은 현재 구현 범위가 아니다. Python 3.13 표준 라이브러리만 사용한다.

## 준비와 실행

서버에 실제 S3 설정과 활성 `dev` 프로파일이 준비되어 있어야 한다. 운영자가 보고 은행의 `banks.is_reporting=true`, 거래 기준일을 포함하는 `bank_reporting_periods`의 적용 기간, `banks.report_format=AML17`을 사전 등록해야 한다. 적용 시작일·종료일은 포함하며 종료일이 없으면 이후 기간에 적용한다. 거래에 등장하는 참조 은행의 존재만으로 보고 자격이 생기지 않는다. 미등록 은행·기준일은 403 `REPORTING_NOT_REGISTERED`다.

API 키 없이 필수 `--bank-id`로 은행 코드(0~2147483647)를 전달한다. 누구든 은행 코드를 지정할 수 있는 테스트용 식별이며 실제 로그인을 대신하지 않는다. `local`도 허용하지만 `prod`가 함께 활성화되면 차단한다. 기본/미지정/그외 프로파일도 차단한다. PowerShell에서 저장소 루트 기준으로 실행한다.
...
4. 동일한 `X-Bank-Id`로 `GET /api/v1/bank/uploads/{uploadId}`를 호출하여 2초마다 최대30분 보고 수신 처리 결과를 확인한다. 완료 응답이 이미 최종 상태면 추가 조회 없이 결과를 출력한다. 목업은 거래 통합 완료까지 자동 폴링하지 않는다. 이후 통합 상태는 같은 은행 코드와 `--upload-id`로 재조회한다.

보고 검수·저장 완료 후 통합 대기 결과 예:

```text
파일명: transactions_2026-09-08.csv / 기준일: 2026-09-08
업로드 시각: 2026-09-09T15:00:00+09:00 / 처리 완료 시각: 2026-09-09T15:00:01+09:00
파일 행 수: 100 / 통합 연결 행 수: 0
결과: 보고 검수·저장 완료, 거래 통합 대기
통합·분석 상태는 업로드 번호로 다시 조회하세요.
```

`receivedAt`은 서버가 객체 수신을 확인한 시각, `finishedAt`은 보고 저장/검수 종료 시각이다. `insertedCount`는 정상 통합 거래에 연결된 해당 보고의 행 수이며 통합 전에는 0이다. 확인할 수 없는 행 수·시각은 '확인되지 않음'으로 표시한다. S3 PUT 성공만으로 보고 저장 완료를 보고하지 않는다.

- `COMPLETED`: 보고 검수·저장 완료, 종료0. 보고 수신 처리의 종료이며 거래 통합·분석 완료를 보장하지 않는다.
- `VALIDATION_FAILED`: 오류의 행·컬럼·사유를 표시하고 수정 후 재업로드 안내, 종료1. 필수결측·형식·기준일 불일치 등 파일 자체 오류는 파일 전체를 보류하며 정상 개체·계좌·거래를 만들지 않는다. 선택 라벨 결측은 허용한다. 오류행0은 정확한 행을 특정할 수 없는 파일 단위 오류다.
- `FAILED`: 서버 처리 오류로 관리자에게 작업번호로 상태 확인 요청, 종료1. 통합 연결 행 수는 서버 응답을 표시하며 임의로 0건이라고 표시하지 않는다.
...
`COMPLETED`일 때 `integrationStatus`별 표시는 다음과 같다. 보고 저장 전에는 `null`이며, 알 수 없는 값은 ‘보고 수집 완료, 통합 상태 재조회 필요’로 표시한다.

| integrationStatus | 목업 표시 |
|---|---|
...
- API 키 환경변수는 서버와 목업 모두 필요 없다. URL 발급·완료 통지·결과 조회에는 보고 은행과 거래 기준일의 사전 등록이 필요하다. 현재 공통 보고 양식은 `AML17`이며 준비 절의 등록 조건을 따른다.
- `BANK_IDENTITY_DISABLED`(403)는 허용되지 않은 서버 프로파일, `VALIDATION_FAILED`(400)는 잘못된 은행 코드 등 요청 오류다. 운영에서는 임시 식별을 사용할 수 없다.
- 은행 측은 데이터 공급 목업만 제공한다. 은행 직원 웹·로그인은 현재 구현 범위에 포함하지 않는다.
...
- 실제 AWS·배포 검증은 사용자 commit/push/dev merge 이후 수행한다. 로컬 테스트 통과를 실제 S3 관통 성공으로 해석하지 않는다. 저장 파일 대조·체크섬 거절·권한·만료·보고 검수/저장 결과와 통합 상태 재조회 확인이 필요하다. `COMPLETED`만으로 거래 통합·분석 완료를 판정하지 않는다.
