...
[단일 HTML 실행](AML-RADAR-v13.html). 브라우저에서 열면 된다. React·CSS·아이콘·컴포넌트 코드를 한 파일에 포함하며 런타임 CDN 의존성이 없다. 개발 원본은 이 폴더의 `src/`다.
...
- Figma의 복제 킷은 이미 존재한다. 코드의 shadcn 소스는 별도이며, 이번 v13에 공식 CLI `shadcn@4.21.0 add`로 가져왔다. 실제 Figma 파일과 v1~12 SVG는 수정하지 않았다.
...
- 30계좌·33연결 관계 그래프. 위험도 필터, 노드 선택과 1~4 hop, 계좌 검색 후 Enter, 거래선 선택과 계좌쌍 패널, 금액 굵기/위험도 색, 단일·두 건·집계선 표현, pan/zoom/화면 맞춤/전체화면, 내부 거래 스크롤.
- 조사 도우미의 현재 표시 방식 button과 sidebar/floating/전체화면 선택 menu, 후속 질문. **응답은 스크립트 예시**이며 실제 모델 분석과 외부 전송은 없다.
- 알림 읽음, 시스템/라이트/다크 테마, 시간대·날짜 형식·목록 기본값·알림 설정, 프로필 이미지의 화면 내 변경, 현재/다른 세션 카드와 로그아웃 확인.
- 상단 `v13 · 시연`에서 정상/로딩/빈 결과/첫 조회 실패/재조회 실패를 선택한다. 재조회 실패는 기존 목록을 유지한다.
...
빌드는 `dist/index.html`과 동일한 `AML-RADAR-v13.html`을 만든다. 검증 당시 파일 크기는 약 1.07 MB다.

2026-09-16 확인:

- TypeScript 검사와 production build 통과. 자동 테스트 9개 통과: AND 필터, 엔티티/연결 무결성, 노드·edge 상호 배타 선택, hop/금액 scale, 담당자·사유 종결 조건, 실제 Calendar 렌더의 28·29·30·31일 연속성.
- 브라우저에서 개인/기관 전환, 전역 검색, filter preset과 chip, Dashboard `전체 목록`의 무필터 이동, 단일 기간 popover와 2026년 8월 날짜 31개, 노드 선택→hop 활성화, 거래선 선택→노드 해제/거래 상세, 임시 저장·종결·이력의 화면 이동 후 유지, 도우미의 단일 현재 모드 button과 3가지 선택지, 설정 항목, 다른 세션 로그아웃, stale 목록 유지/재시도, L2 Episode 판단 선택을 확인했다.
