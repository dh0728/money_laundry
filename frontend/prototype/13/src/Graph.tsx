...
import { Expand, Scan, Minus, Plus, Search, X, Network, RotateCcw } from 'lucide-react'
...
export default function Graph(){
  ...
  const body=<div className={`graph-layout ${fullscreen?'h-full min-h-0':'h-[640px]'} overflow-hidden rounded-lg border bg-card`}>
    <ResizablePanelGroup orientation="horizontal">
      <ResizablePanel defaultSize="18%" minSize="155px" maxSize="240px">
        <div className="p-4 h-full flex flex-col gap-5"><div className="flex justify-between items-center"><h3 className="font-semibold text-sm">그래프 설정</h3><IconButton label="그래프 초기화" onClick={reset}><RotateCcw className="size-3.5"/></IconButton></div>
          ...
          <div className="space-y-3"><Label className="text-xs text-muted-foreground">계좌 위험도</Label>{(Object.keys(riskColor) as Risk[]).map(r=><Label key={r} className="flex items-center gap-2 text-xs font-normal"><Checkbox checked={risks.includes(r)} onCheckedChange={v=>{setRisks(p=>v?[...p,r]:p.filter(x=>x!==r));setSelection(s=>({...s,edge:null}))}}/><span className="size-2 rounded-full" style={{background:riskColor[r]}}/>{r}</Label>)}</div>
          <div className="space-y-3"><div className="flex items-center justify-between"><Label className="text-xs text-muted-foreground">표시 범위</Label><Badge variant="outline">{selection.hop} hop</Badge></div><Slider aria-label="표시 범위 hop" min={1} max={4} step={1} value={[selection.hop]} disabled={!selection.node} onValueChange={([hop])=>setSelection(s=>({...s,hop}))}/><div className="flex justify-between text-[10px] text-muted-foreground"><span>1</span><span>2</span><span>3</span><span>4</span></div><p className="text-[11px] leading-relaxed text-muted-foreground">{selection.node?`${selection.node}에서 연결된 계좌를 표시합니다.`:'계좌를 선택하면 표시 범위를 조절할 수 있습니다.'}</p></div>
      ...
      <ResizablePanel minSize="300px">
        <div className="h-full flex flex-col min-w-0"><div className="flex gap-2 p-3 border-b items-center"><div className="relative min-w-0 flex-1"><Search className="absolute left-2.5 top-2.5 size-3.5 text-muted-foreground"/><Input aria-label="그래프 계좌 검색" placeholder="계좌 검색 · A01" className="pl-8 h-8 text-xs" value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){const n=nodes.find(n=>n.id.toLowerCase()===search.trim().toLowerCase());if(n){pick('node',n.id);if(!risks.includes(n.risk))setRisks(p=>[...p,n.risk])}}}}/></div><IconButton label="화면 맞춤" onClick={fit}><Scan className="size-4"/></IconButton><IconButton label={fullscreen?'전체화면 종료':'전체화면'} onClick={()=>setFullscreen(!fullscreen)}><Expand className="size-4"/></IconButton></div>
          <div className="relative flex-1 min-h-[500px] graph-canvas"><svg role="img" aria-label={`계좌 관계 그래프 · 계좌 ${visible.length}개, 연결 ${visibleEdges.length}개`} viewBox="0 0 1000 690" className="w-full h-full absolute inset-0 touch-none" onPointerMove={move} onPointerUp={()=>drag.current=null} onPointerCancel={()=>drag.current=null} onPointerDown={e=>{if(e.target===e.currentTarget){drag.current={x:e.clientX,y:e.clientY,vx:view.x,vy:view.y};e.currentTarget.setPointerCapture(e.pointerId)}}} onWheel={e=>{if(e.ctrlKey||e.metaKey)zoom(e.deltaY>0?.9:1.1)}}>
