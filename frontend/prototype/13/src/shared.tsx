...
export function RiskBadge({risk,score}:{risk:Risk;score?:number}){return <Badge variant="outline" className="gap-1.5 font-normal" style={{color:riskColor[risk],borderColor:`${riskColor[risk]}40`}}><span className="size-1.5 rounded-full" style={{background:riskColor[risk]}}/>{risk}{score!==undefined&&<span className="font-mono ml-1">{score}</span>}</Badge>}
...
export function DateRangeButton({value,onChange}:{value:DateRange|undefined;onChange:(r:DateRange|undefined)=>void}){
  ...
  return <Popover open={open} onOpenChange={o=>{setOpen(o);if(o)setDraft(value)}}><PopoverTrigger asChild><Button variant="outline" size="sm" className="font-normal"><CalendarDays className="size-3.5"/>{label}</Button></PopoverTrigger><PopoverContent align="start" className="w-auto p-0"><div className="flex"><div className="w-32 border-r p-2 flex flex-col gap-1">{[['오늘',1],['최근 3일',3],['최근 7일',7],['최근 30일',30]].map(([l,d])=><Button key={l} variant="ghost" size="sm" className="justify-start font-normal" onClick={()=>preset(Number(d))}>{l}</Button>)}<Button variant="ghost" size="sm" className="justify-start font-normal" onClick={()=>setDraft({from:startOfMonth(today),to:today})}>이번 달</Button><Button variant="ghost" size="sm" className="justify-start font-normal" onClick={()=>setDraft(undefined)}>전체 기간</Button></div><Calendar locale={ko} mode="range" selected={draft} onSelect={setDraft} defaultMonth={draft?.from??new Date(2026,7,1)} today={today} numberOfMonths={2}/></div><div className="border-t p-3 flex items-center justify-between gap-6"><span className="text-xs text-muted-foreground">{draft?.from?`${draft.from.toLocaleDateString('sv-SE')} → ${draft.to?.toLocaleDateString('sv-SE')??'종료일 선택'}`:'전체 기간'}</span><div className="flex gap-2"><Button variant="outline" size="sm" onClick={()=>setOpen(false)}>취소</Button><Button size="sm" onClick={()=>{onChange(draft);setOpen(false)}}>적용</Button></div></div></PopoverContent></Popover>
}
