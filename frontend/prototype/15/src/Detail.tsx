...
export default function Detail({ record: r, records, user, onUpdate, onOpen }: { record: RecordItem; records: RecordItem[]; user: string; onUpdate: (r: RecordItem) => void; onOpen: (r: RecordItem) => void }) {
  ...
  return (
    <div className="space-y-5">
      {/* Figma v15c: 가로가 넓은 화면에서 낭비되던 header 공간을 압축 — 좌측 제목 블록과 우측 연결/공지 블록을 한 줄 band로 정렬하고, 좁은 화면에서만 세로로 쌓는다 */}
      <div className="flex flex-col @5xl:flex-row @5xl:items-start @5xl:justify-between gap-3" data-testid="detail-header">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-2.5"><RiskBadge risk={r.risk} score={r.score} /><PatternBadge pattern={r.pattern} probability={r.probability} /><span className="text-xs text-muted-foreground">담당 {r.owner} · {r.date} 탐지</span></div>
        </div>
        {(linkedEpisode || linkedAlerts.length > 0 || !responsible) && (
          <div className="flex flex-col items-start @5xl:items-end gap-2 shrink-0">
            {(linkedEpisode || linkedAlerts.length > 0) && (
              <div className="flex flex-wrap gap-2 items-center @5xl:justify-end">
                {linkedEpisode && <span className="flex items-center gap-1.5 border rounded-md px-2.5 py-1.5 text-xs"><Link2 className="size-3.5" />연결된 Episode<Button variant="link" className="p-0 h-auto text-xs font-mono" onClick={() => onOpen(linkedEpisode)}>{linkedEpisode.id}</Button></span>}
                {linkedAlerts.length > 0 && <span className="flex flex-wrap items-center gap-1.5 border rounded-md px-2.5 py-1.5 text-xs"><Link2 className="size-3.5" />연결된 Alert {linkedAlerts.map(a => <Button key={a.id} variant="link" className="p-0 h-auto text-xs font-mono" onClick={() => onOpen(a)}>{a.id}</Button>)}</span>}
              </div>
            )}
            {!responsible && <p className="text-xs text-muted-foreground rounded-md border px-3 py-2 @5xl:whitespace-nowrap">현재 {user} 계정으로 조회 중입니다. 최종 처리는 담당자 {r.owner}가 수행합니다.</p>}
          </div>
        )}
      </div>
      ...
      {tab === 'overview' && (
        <div className="space-y-5" data-testid="overview">
          <div className="grid grid-cols-2 @3xl:grid-cols-4 gap-4">
            {[{ label: '세탁 거래 금액', value: usd(r.amount) }, { label: '근거 거래', value: `${tx.length}건` }, { label: r.kind === 'Alert' ? '세탁 참여 계좌' : '연결 Alert', value: r.kind === 'Alert' ? `${model.nodes.filter(n => n.core).length}개` : `${linkedAlerts.length}건` }, { label: '경과 기간', value: r.age === 0 ? '오늘' : `${r.age}일` }].map(s => (
          ...
          <div className="grid gap-4 @5xl:grid-cols-[1.3fr_1fr_1fr]">
            <Card className="shadow-none"><CardContent>
              <SectionTitle title="일별 세탁 거래 금액" description="언제 집중됐는지 · USD" />
          ...
          <div className="grid gap-4 @5xl:grid-cols-[1fr_1.4fr_1fr]">
            <Card className="shadow-none"><CardContent>
              <SectionTitle title="세탁 거래 모양" description={`${r.pattern} 유형 도식 · 실제 계좌는 자금 흐름에서 확인`} />
            ...
            <Card className="shadow-none"><CardContent>
              ...
              <div className="space-y-5 text-sm">
                <div className="flex gap-3"><span className="text-muted-foreground font-mono text-xs pt-0.5">01</span><div><p className="font-medium">{r.title}</p><p className="text-xs text-muted-foreground leading-6 mt-1">세탁 라벨 거래 {tx.length}건, 참여 계좌 {model.nodes.filter(n => n.core).length}개가 탐지 범위에 포함됨. 거래 목적 및 계좌 간 관계 확인 필요.</p></div></div>
              ...
              <Table>
                ...
                <TableBody>{rows.map(t => (
                  <TableRow key={t.id} data-state={picked?.id === t.id ? 'selected' : undefined} className="cursor-pointer" onClick={() => setSelectedTx(t.id)}>
                    <TableCell className="font-mono text-xs">{t.id}</TableCell><TableCell className="text-xs tabular-nums">{t.at}</TableCell>
                    <TableCell className="font-mono text-xs">{t.from}</TableCell><TableCell className="font-mono text-xs">{t.to}</TableCell>
                    <TableCell className="text-right text-xs tabular-nums">{usd(t.usd)}</TableCell><TableCell className="text-xs">{t.format}</TableCell>
                  </TableRow>
                ))}</TableBody>
              </Table>
            </div>
        ...
        <div className="grid gap-4 @5xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,1fr)] items-start">
          <Card className="shadow-none"><CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Textarea id="reason" value={reason} onChange={e => setReason(e.target.value)} disabled={closed || !responsible} className="min-h-44 text-sm leading-7" placeholder="확인한 거래, 계좌 간 관계, 판단 근거를 작성하세요." />
            </div>
            <div className="flex items-center justify-end gap-2 border-t pt-5">
              <Button variant="ghost" size="sm" onClick={() => setDraftOpen(true)}><FileClock className="size-3.5" />저장본 목록 {drafts.length > 0 && `(${drafts.length})`}</Button>
          ...
          <Card className="shadow-none"><CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div><p className="text-muted-foreground">세탁 거래</p><p className="mt-1.5 text-base font-semibold tabular-nums">{tx.length}건 · {compactUsd(r.amount)}</p></div>
