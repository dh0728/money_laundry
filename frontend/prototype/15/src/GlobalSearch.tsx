...
// (v15c-shell.test.tsx가 검색 색인을 직접 단위 테스트할 수 있도록 export한다)
export const pageLinks = [
  { page: 'dashboard', label: '대시보드' },
  { page: 'alerts', label: 'Alert' },
  { page: 'episodes', label: 'Episode' },
...
export default function GlobalSearch({ records, onOpenRecord, onNavigate }: { records: RecordItem[]; onOpenRecord: (r: RecordItem) => void; onNavigate: (page: string) => void }) {
  const [open, setOpen] = useState(false), [query, setQuery] = useState(''), [active, setActive] = useState(0)
  const accountIndex = useMemo(() => buildAccountIndex(records), [records])
  ...
  const groups: ResultGroup[] = useMemo(() => {
    ...
    const recordItems: ResultItem[] = records
      .filter(r => `${r.id} ${r.title} ${r.pattern} ${r.owner}`.toLowerCase().includes(q))
      .slice(0, 5)
      .map(r => ({
        key: `record-${r.id}`,
    ...
    const accountItems: ResultItem[] = accountIndex
      .filter(e => e.account.toLowerCase().includes(q) || e.entity.toLowerCase().includes(q))
      .flatMap(e => e.recordIds.map(id => ({ entry: e, id })))
      .slice(0, 5)
      .map(({ entry, id }) => {
        const record = records.find(r => r.id === id)
        return {
          key: `account-${entry.account}-${id}`,
          primary: <span className="font-mono text-xs"><Highlight text={entry.account} query={q} /></span>,
          secondary: `${entry.entity} · ${entry.bank} · ${id}`,
          onSelect: () => { if (record) select(() => onOpenRecord(record)) },
        }
      })
    ...
    return [
      { id: 'record', label: 'Alert·Episode', items: recordItems },
      { id: 'account', label: '계좌', items: accountItems },
      { id: 'noti', label: '알림', items: notificationItems },
      { id: 'screen', label: '화면·설정', items: settingsItems },
    ].filter(g => g.items.length > 0)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [q, records, accountIndex, notifications])
  ...
  return (
    <Popover open={open} onOpenChange={o => { setOpen(o); if (!o) setActive(0) }}>
      <PopoverTrigger asChild>
        <div className="relative">
          ...
          <Input
            ...
            placeholder="Alert ID · Episode ID · 계좌 · 알림 · 설정 검색" className="h-9 pl-9 pr-10 bg-muted/40" value={query}
      ...
      <PopoverContent id="global-search-listbox" role="listbox" aria-label="검색 결과" className="w-[420px] max-w-[90vw] p-2" onOpenAutoFocus={e => e.preventDefault()}>
        {flat.length === 0
