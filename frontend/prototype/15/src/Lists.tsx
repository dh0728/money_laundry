      ...
      <TableBody>
        {sortRows(rows, sort.key, sort.direction).slice(page * pageSize, (page + 1) * pageSize).map(r => (
          <TableRow key={r.id} className="cursor-pointer" tabIndex={0} onClick={() => onOpen(r)} onKeyDown={e => { if (e.key === 'Enter') onOpen(r) }}>
            <TableCell><p className="text-xs font-mono">{r.id}</p><p className="text-[11px] text-muted-foreground mt-1 truncate">{r.title}</p></TableCell>
            <TableCell><RiskBadge risk={r.risk} score={r.score} /></TableCell>
            <TableCell><PatternBadge pattern={r.pattern} probability={r.probability} /></TableCell>
            <TableCell className="text-right tabular-nums text-xs">{usd(r.amount)}</TableCell>
            <TableCell className="text-right tabular-nums text-xs">{r.count}건</TableCell>
            <TableCell className="text-xs">{r.owner}</TableCell>
            <TableCell><Badge variant={r.status === '신규' ? 'secondary' : 'outline'} className="font-normal text-[11px]">{r.status}</Badge></TableCell>
            <TableCell className="text-xs text-muted-foreground tabular-nums">{r.date.slice(5)}<p className={`mt-1 ${r.age >= 3 ? 'font-medium' : ''}`} style={{ color: ageTone(r.age) }}>{r.age === 0 ? '오늘' : `${r.age}일 경과`}</p></TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
