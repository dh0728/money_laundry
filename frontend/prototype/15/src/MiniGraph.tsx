...
// 세탁 거래(라벨 1)에 참여한 계좌와 그 사이 엣지만 그린 요약 모양. 개요·기관 현황판에서 쓴다.
export default function MiniGraph({ model, className = '' }: { model: GraphModel; className?: string }) {
  ...
  return (
    <svg viewBox={`${cx - span / 2} ${cy - span / 2} ${span} ${span}`} className={className} role="img" aria-label={`세탁 거래 계좌 ${core.length}개, 연결 ${edges.length}개`}>
