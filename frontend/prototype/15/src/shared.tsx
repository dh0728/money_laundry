...
export function RiskBadge({ score }: { risk: Risk; score: number }) {
  ...
  return (
    <Badge variant="outline" className="gap-1.5 font-normal" style={{ color: tone, borderColor: `color-mix(in oklch, ${tone} 40%, transparent)` }}>
      <span className="font-mono">{score}</span><span className="text-[10px] opacity-75">상위 {topPercent(score)}%</span>
    </Badge>
  )
}
...
export function SortableHead({ label, active, direction, onSort, align = 'left', className }: { label: string; active: boolean; direction: SortDirection; onSort: () => void; align?: 'left' | 'right'; className?: string }) {
  ...
  return (
    <TableHead className={className} aria-sort={state === 'desc' ? 'descending' : state === 'asc' ? 'ascending' : 'none'}>
      <button type="button" onClick={onSort} aria-label={`${label} 정렬 · 현재 ${spoken}`} className={`group inline-flex w-full items-center gap-2 text-xs font-medium ${align === 'right' ? 'justify-end' : 'justify-start'} ${state ? 'text-foreground' : ''}`}>
        {label}<SortIcon direction={state} />
      </button>
    </TableHead>
  )
}
