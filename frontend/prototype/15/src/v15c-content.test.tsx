...
describe('Figma v15c · 쨍한 레드 위험도 스케일', () => {
...
describe('Figma v15c · 경과일(age) 히트 색상', () => {
...
describe('Figma v15c · 알림 카드 rail(좌측 dot + 되돌리기)', () => {
  ...
  it('카드는 44px 폭 rail을 가진 grid이고, dot과 되돌리기 버튼을 포함한다', () => {
    const markup = html(<Notifications records={records} onOpen={() => {}} />)
    expect(markup).toContain('notification-card grid grid-cols-[44px_1fr]')
...
describe('Figma v15c · Detail 헤더 압축', () => {
  ...
  it('연결된 Episode/Alert 안내는 여전히 button이 아니라 텍스트로 표시된다', () => {
    expect(markup).not.toMatch(/<button[^>]*>[^<]*<svg[^>]*>.*?<\/svg>연결된 (Episode|Alert)/)
  })
})
