...
import { edgeGeometry, fitView } from './Graph'
...
describe('팀원 데이터 기반 관계 그래프', () => {
  ...
  it('모든 Alert는 유형이 같은 블록의 세탁 거래로 금액·건수를 계산한다', () => {
  ...
  it('금액이 클수록 선·화살촉이 패널 안 min~max 기준으로 확연히 굵어진다', () => {
    const amounts = [10, 1_000, 100_000, 10_000_000]
    const widths = amounts.map(a => widthFor(a, 10, 10_000_000))
    expect(widths[0]).toBeCloseTo(1.2, 5); expect(widths[3]).toBeCloseTo(14, 5)
    expect(widths.every((w, i) => i === 0 || w > widths[i - 1])).toBe(true)
    expect(widths[3] / widths[0]).toBeGreaterThan(3)
    expect(widthFor(5, 5, 5)).toBeCloseTo((1.2 + 14) / 2, 5) // min===max: 중간값
    // 하나의 채운 폴리곤(몸통+화살촉)에서 화살촉 폭(head width ≈ w*1.8+6)도 굵기에 따라 뚜렷하게 넓어진다
    const headSpan = (w: number) => {
      const nums = edgeGeometry({ x: 0, y: 0 }, { x: 300, y: 0 }, 5, 5, w).polygon.match(/-?[\d.]+/g)!.map(Number)
      // d = M s1 L b1 L h1 L tip L h2 L b2 Z → h1은 인덱스4·5, h2는 인덱스8·9
      return Math.hypot(nums[4] - nums[8], nums[5] - nums[9])
    }
    expect(headSpan(8)).toBeGreaterThan(headSpan(2))
    expect(headSpan(8)).toBeCloseTo(8 * 1.8 + 6, 5)
  })
  it('화면 맞춤은 모든 계좌를 여백 안에 넣는다', () => {
    const { nodes } = graphFor(records[5]), view = fitView(nodes, 900, 600)
    for (const n of nodes) {
      const x = n.x * view.k + view.x, y = n.y * view.k + view.y
      expect(x).toBeGreaterThanOrEqual(55); expect(x).toBeLessThanOrEqual(845)
      expect(y).toBeGreaterThanOrEqual(55); expect(y).toBeLessThanOrEqual(545)
    }
  })
