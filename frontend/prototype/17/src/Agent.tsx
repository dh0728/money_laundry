...
import { Send, PanelRight, AppWindow, X, Plus, Menu } from 'lucide-react'
...
const HISTORY_DEFAULT = 224, HISTORY_MIN = 200, HISTORY_MAX = 420
...
export function AgentToggle({ open, onToggle }: { open: boolean; onToggle: () => void }) {
  return (
    <Button type="button" variant={open ? 'secondary' : 'outline'} size="sm" aria-pressed={open} aria-label={open ? `${AGENT_NAME} 닫기` : `${AGENT_NAME} 열기`} data-testid="agent-toggle"
      className="agent-toggle h-9 shrink-0 rounded-full pl-1 pr-3.5 gap-2 font-mono text-xs tracking-wide max-[1100px]:w-9 max-[1100px]:px-0 max-[1100px]:justify-center" onClick={onToggle}>
      <RadarSweep className="size-7 shrink-0" glow={open} /><span className="max-[1100px]:hidden">{AGENT_NAME}</span>
    </Button>
  )
}
...
export default function Agent({ open, setOpen, mode, setMode, record, records }: { open: boolean; setOpen: (o: boolean) => void; mode: AgentMode; setMode: (m: AgentMode) => void; record?: RecordItem; records: RecordItem[] }) {
  ...
  const [floatSize, setFloatSize] = useState({ width: FLOAT_W_DEFAULT, height: FLOAT_H_DEFAULT })
  ...
  const moveFloatResize = (e: PointerEvent<HTMLDivElement>) => {
    ...
    if (!r) return
    const maxW = Math.max(FLOAT_MIN_W, window.innerWidth * 0.9), maxH = Math.max(FLOAT_MIN_H, window.innerHeight * 0.85)
    setFloatSize(prev => ({
      width: r.axis === 'y' ? prev.width : clamp(r.width + (e.clientX - r.x), FLOAT_MIN_W, maxW),
      height: r.axis === 'x' ? prev.height : clamp(r.height + (e.clientY - r.y), FLOAT_MIN_H, maxH),
    }))
  }
  ...
  const modes = [{ value: 'sidebar', label: '우측 사이드바', icon: PanelRight }, { value: 'floating', label: '플로팅 패널', icon: AppWindow }] as const
  ...
  const moveDrag = (e: PointerEvent<HTMLDivElement>) => {
    ...
    setPosition({ x: Math.max(8, Math.min(window.innerWidth - floatSize.width - 8, drag.current.left + e.clientX - drag.current.x)), y: Math.max(8, Math.min(window.innerHeight - floatSize.height - 8, drag.current.top + e.clientY - drag.current.y)) })
  }
  if (!open) return null
  ...
  const history = (
    <aside data-testid="agent-history" data-open={historyOpen} aria-hidden={!historyOpen} inert={!historyOpen}
      className={`agent-history absolute top-0 bottom-0 right-full z-0 flex flex-col border bg-background shadow-xl rounded-l-lg ${historyOpen ? 'is-open' : ''}`}
