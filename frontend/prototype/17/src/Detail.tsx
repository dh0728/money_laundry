    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col @5xl:flex-row @5xl:items-start @5xl:justify-between gap-3" data-testid="detail-header">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-2.5" data-testid="detail-row2">
            ...
            <span className="text-xs text-muted-foreground">담당 {r.owner} · {r.date} 탐지</span>
          </div>
        </div>
        {!responsible && (
      ...
      {tab === 'conclusion' && (
        <div className="grid gap-4 @5xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,1fr)] items-start">
          {/* Figma v17: textarea를 처음부터 크게 잡고, 카드는 self-stretch + flex-col로 옆 카드(판단 참고) 높이까지 채워 textarea가 남는 높이를 흡수한다 — 폼 아래 큰 빈 공간 제거 */}
          <Card className="shadow-none @5xl:self-stretch"><CardContent className="flex flex-col gap-6 h-full">
            ...
            <div className="space-y-2 flex-1 flex flex-col min-h-0">
              ...
              <Textarea id="reason" value={reason} onChange={e => setReason(e.target.value)} disabled={closed || !responsible} className="flex-1 min-h-[280px] resize-none text-sm leading-7" placeholder="확인한 거래, 계좌 간 관계, 판단 근거를 작성하세요." />
            </div>
