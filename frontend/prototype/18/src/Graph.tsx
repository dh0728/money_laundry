...
// 직선 엣지: 균일한 굵기의 몸통 선(stroke) + 별도의 작은 화살촉(fill polygon).
// 시작·끝은 각 노드 경계에 붙고, v18 G3: 화살촉은 몸통 정중앙에 둔다.
...
export function edgeGeometry(a: Point, b: Point, ra: number, rb: number, width: number) {
  ...
  const mid = { x: (start.x + tip.x) / 2, y: (start.y + tip.y) / 2 }
  // v18 G3: 화살촉을 도착점이 아니라 몸통 정중앙에 둔다
  const headLen = Math.min(4 + width * .9, Math.max(shaftLen / 2 - 1, 1))
  const halfHead = headLen * .62
  const arrowTip = { x: mid.x + ux * (headLen / 2), y: mid.y + uy * (headLen / 2) }
  const base = { x: mid.x - ux * (headLen / 2), y: mid.y - uy * (headLen / 2) }
  return {
    full: `M${start.x},${start.y} L${tip.x},${tip.y}`, // hover/hit 영역용 전체 길이
    line: `M${start.x},${start.y} L${tip.x},${tip.y}`, // 균일 굵기 몸통(화살촉은 중앙에 겹침)
    arrow: `M${arrowTip.x},${arrowTip.y} L${base.x + nx * halfHead},${base.y + ny * halfHead} L${base.x - nx * halfHead},${base.y - ny * halfHead} Z`,
    start, tip, mid, arrowTip, base,
  }
}
...
export default function Graph({ model, label }: { model: GraphModel; label: string }) {
  ...
  // v18 G6/G7: 중간 분할·상세 패널 폭은 ResizablePanel로 조절(전체화면·좁은 화면도 동일).
  ...
  // v18 G5: 상세는 본문을 덮지 않고 밀어낸다(overlay/drawer 금지). G6/G7: 중간 분할·상세 폭 리사이즈.
    ...
    <div className="flex-1 min-h-0 flex" data-testid={testId} data-split="horizontal">
      <ResizablePanelGroup orientation="horizontal" className="flex-1 min-h-0">
        ...
        {panel && (
          <>
            <ResizableHandle />
            <ResizablePanel key={focus?.kind ?? 'none'} id={`${testId}-detail`} defaultSize={`${panelBounds.def}px`} minSize={`${panelBounds.min}px`} maxSize={`${panelBounds.max}px`}>
    ...
    <div className="flex-1 min-h-0 flex flex-col" data-testid="graph-split-v" data-split="vertical">
      ...
      <ResizablePanelGroup orientation="vertical" className="flex-1 min-h-0">
        ...
        {panel && (
          <>
            <ResizableHandle />
            <ResizablePanel key={focus?.kind ?? 'none'} minSize="180px" defaultSize="40%" id="graph-detail-v">
              <div className="h-full min-h-0" data-testid="graph-detail-panel">{panel}</div>
            </ResizablePanel>
          </>
        )}
