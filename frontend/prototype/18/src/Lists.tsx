...
// T3: 행 패딩·본문 글자를 한 단계 키움. 목록 패널 높이는 고정하지 않고 레이아웃/리사이즈에 맡긴다.
      ...
      <TableBody>
        {sortRows(rows, sort.key, sort.direction).slice(page * pageSize, (page + 1) * pageSize).map(r => {
          ...
          <TableRow key={r.id} className="cursor-pointer" tabIndex={0} onClick={() => onOpen(r)} onKeyDown={e => { if (e.key === 'Enter') onOpen(r) }}>
            <TableCell data-sorted={sorted('id')} className="!py-4"><p className="text-sm font-mono">{r.id}</p><p className="text-xs text-muted-foreground mt-1 truncate">{r.title}</p></TableCell>
            <TableCell data-sorted={sorted('score')} className="!py-4"><RiskBadge risk={r.risk} score={r.score} /></TableCell>
            <TableCell data-sorted={sorted('pattern')} className="!py-4"><PatternBadge pattern={r.pattern} probability={r.probability} /></TableCell>
            <TableCell data-sorted={sorted('amount')} className="!py-4 text-right tabular-nums text-sm">{usd(r.amount)}</TableCell>
            <TableCell data-sorted={sorted('count')} className="!py-4 text-right tabular-nums text-sm">{r.count}건</TableCell>
            <TableCell data-sorted={sorted('owner')} className="!py-4 text-sm">{r.owner}</TableCell>
            <TableCell data-sorted={sorted('status')} className="!py-4"><Badge variant={r.status === '신규' ? 'secondary' : 'outline'} className="font-normal text-xs">{r.status}</Badge></TableCell>
            <TableCell data-sorted={sorted('date')} className="!py-4 text-sm text-muted-foreground tabular-nums">{r.date.slice(5)}<p className={`mt-1 text-xs ${r.age >= 3 ? 'font-medium' : ''}`} style={{ color: ageTone(r.age) }}>{r.age === 0 ? '오늘' : `${r.age}일 경과`}</p></TableCell>
          </TableRow>
          )
        })}
      </TableBody>
