...
describe('팀원 데이터 기반 관계 그래프', () => {
  ...
  it('금액이 클수록 선(균일 굵기)·화살촉이 패널 안 min~max 기준(로그 척도)으로 확연히 굵어진다', () => {
    ...
    // v17: 균일 굵기 stroke(line) + 별도의 작은 화살촉(arrow)으로 분리되었다. 화살촉 폭도 굵기에 따라 넓어진다.
    const g = edgeGeometry({ x: 0, y: 0 }, { x: 300, y: 0 }, 5, 5, 8)
    expect(g.line.startsWith('M')).toBe(true)
    expect(g.arrow.trim().endsWith('Z')).toBe(true)
    const headSpan = (w: number) => {
      const nums = edgeGeometry({ x: 0, y: 0 }, { x: 300, y: 0 }, 5, 5, w).arrow.match(/-?[\d.]+/g)!.map(Number)
      // d = M tip L h1 L h2 Z → h1은 인덱스2·3, h2는 인덱스4·5
      return Math.hypot(nums[2] - nums[4], nums[3] - nums[5])
    }
