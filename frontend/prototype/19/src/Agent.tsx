...
export const peekFabDragBounds = () => {
  ...
  const maxLeft = Math.max(minLeft, (globalThis.innerWidth ?? 1440) - FAB_SIZE - 8)
...
export const fabHomeRect = (): FabOrigin => {
  ...
  return { left: vw - 24 - FAB_SIZE, top: vh - 24 - FAB_SIZE, width: FAB_SIZE, height: FAB_SIZE }
}
      ...
      <ScrollArea className="flex-1 min-h-0">
        <div className="p-4 space-y-5">
          <div className="space-y-3">
            <div ref={avatarRef} className={`rdr-avatar size-14 shrink-0 ${avatarVisible ? "opacity-100" : "opacity-0"}`}><RadarSweep className="size-14 block" /></div>
