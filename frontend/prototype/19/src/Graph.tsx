...
const dim = (hex: string) => hex.startsWith('#') && hex.length === 7 ? `${hex}33` : hex
export default function Graph({ model, label }: { model: GraphModel; label: string }) {
  ...
  // 클릭·hop 선택은 물리 배치에 영향을 주지 않는다: 시뮬레이션 데이터는 모델이 바뀔 때만 새로 만든다(범위 밖은 흐리게만).
  ...
  const colors = { l1: cssVar('--graph-l1'), l0: cssVar('--graph-l0'), node: cssVar('--graph-l0-node'), fg: cssVar('--foreground'), muted: cssVar('--muted-foreground') }
  const q = search.trim().toLowerCase()
  const nodeVisible = (n: FNode) => (nodeStart.get(n.key) ?? -Infinity) <= current
  // hop 범위 밖·hover 비이웃은 숨기지 않고 흐리게(모양 유지)
  const faded = (key: string) => (allowed && !allowed.has(key)) || (lit && !lit.has(key))
  const linkVisible = (l: FLink) => edgeStart(l as GraphEdge) <= current
  ...
  const forceGraph = size.w > 0 && size.h > 0 && (
    <ForceGraph2D<GraphNode & { id: string }, GraphEdge>
      ...
      linkColor={l => { const c = l.label === 1 ? colors.l1 : colors.l0; return linkLit(l) ? c : dim(c) }}
      ...
      // 의심 거래 선 위로 입자가 흘러 자금 이동 방향을 보여준다
      linkDirectionalParticles={l => l.label === 1 ? 2 : 0} linkDirectionalParticleWidth={l => Math.max(2, widthFor(l.usd, usdMin, usdMax))} linkDirectionalParticleColor={() => colors.l1}
  ...
  const mainGraph = (
    <div className="h-full flex flex-col min-w-0" tabIndex={-1} onKeyDown={onKeyDown} data-testid="graph-region">
      <div className="flex gap-2 p-3 border-b items-center shrink-0">
        ...
        <IconButton label="화면 맞춤" disabled={fitted} onClick={() => fitGraph()}><Shrink className="size-4" /></IconButton>
      ...
      <div ref={canvasRef} className="relative flex-1 min-h-0 graph-canvas outline-none" tabIndex={0} role="img" aria-label={`${label} · 계좌 ${nodes.length}개, 연결 ${edges.length}개 · ←/→ 이전·다음 거래, Space 재생`} onPointerMove={e => { const r = e.currentTarget.getBoundingClientRect(); pointer.current = { x: e.clientX - r.left, y: e.clientY - r.top } }}>
        ...
        {hover && (hoverNode || hoverEdge) && (
          <div className="graph-tooltip" style={{ left: Math.min(hover.x + 14, size.w - 260), top: Math.min(hover.y + 14, size.h - 110) }}>
            {hoverNode && <><b>{hoverNode.entity}</b><br />{hoverNode.account} · Bank {hoverNode.bank}<br />{hoverNode.core ? '의심 거래 참여 계좌' : hoverNode.hub ? `허브 · 상대 계좌 ${hoverNode.hubDegree.toLocaleString()}개` : hoverNode.bridge ? '연결 경로 경유 계좌' : `${hoverNode.hop}단계 주변 계좌`}{hoverNode.synthetic && ' · 합성 예시'}</>}
