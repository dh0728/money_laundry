...
describe('v18 shell fixes from 명기', () => {
  ...
  it('로그인 오른쪽은 글래스모피즘(반투명+blur)·인풋 불투명·제목/화살표 없음', () => {
    ...
    expect(css).toMatch(/\.login-glass-panel\s*\{[\s\S]*?background:\s*rgba\(255,\s*255,\s*255,\s*0\.035\)/)
    expect(css).toMatch(/\.login-input-opaque\s*\{[\s\S]*?background-color:\s*rgb\(22,\s*22,\s*28\)/)
  })
})
