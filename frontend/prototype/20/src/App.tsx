...
import { LayoutDashboard, Siren, FolderSearch, Bell, Settings as SettingsIcon, ArrowLeft, ListTodo, Maximize2, Minimize2, PanelLeft, Eye, EyeOff } from 'lucide-react'
    ...
    <div className="login-screen relative min-h-screen overflow-hidden bg-[#070709] text-white">
      ...
      <div className="relative z-10 min-h-screen grid grid-cols-[1fr_minmax(380px,1fr)] login-layout">
        <div className="flex flex-col justify-between p-12 pointer-events-none">
          ...
          <p className="text-[11px] text-white/45">AML RADAR · v20</p>
        </div>
        {/* 오른쪽 절반 = css.glass식 글래스모피즘(반투명+blur). 인풋만 불투명. 제목/화살표 없음 */}
        <div className="login-glass-panel relative flex items-center justify-center p-8 pointer-events-auto">
...
export default function App() {
  ...
  const go = (p: Page) => { setPage(p); setSelected(null); setState(initialState); if (p === 'alerts' || p === 'episodes') setListKey(k => k + 1); document.querySelector(".app-main")?.scrollTo(0, 0) }
  const openRecord = (r: RecordItem) => { setPage(r.kind === 'Alert' ? 'alerts' : 'episodes'); setSelected(r.id); document.querySelector(".app-main")?.scrollTo(0, 0) }
