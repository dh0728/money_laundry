...
export default function Detail({ record: r, records, user, onUpdate, onOpen }: { record: RecordItem; records: RecordItem[]; user: string; onUpdate: (r: RecordItem) => void; onOpen: (r: RecordItem) => void }) {
  ...
  const [selectedTx, setSelectedTx] = useState<string | null>(null), [selectedTxIndex, setSelectedTxIndex] = useState(0)
  ...
  return (
    <div className="min-h-full flex flex-col gap-5">
      ...
      {tab === 'transactions' && (() => {
        const picked = tx.find(t => t.id === selectedTx) ?? [...tx].sort((a, b) => a.at.localeCompare(b.at))[0]
        return (
          <div className="grid gap-4 @5xl:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)] items-start">
            <TxTable rows={tx} selectedId={picked?.id} onSelect={setSelectedTx} onSelectedIndexChange={setSelectedTxIndex} />
            <Card className="selected-transaction-card shadow-none" style={{ '--selected-row-offset': `${42 + selectedTxIndex * 49}px` } as CSSProperties}><CardContent className="space-y-6">
