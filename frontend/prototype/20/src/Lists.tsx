...
export default function Lists({ kind, records, user, onOpen, state, setState }: { kind: Kind; records: RecordItem[]; user: string; onOpen: (r: RecordItem) => void; state: DataState; setState: (s: DataState) => void }) {
  ...
  const { table } = useDataTable({
    data: result, columns, pageCount: -1, clientSide: true, getRowId: r => r.id, defaultColumn: { enableHiding: false },
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Popover open={filterOpen} onOpenChange={setFilterOpen}>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm" className="h-9"><ListFilter data-icon="inline-start" />필터{filters.length > 0 && <Badge className="ml-1 h-5 min-w-5 px-1.5">{filters.length}</Badge>}</Button>
          </PopoverTrigger>
