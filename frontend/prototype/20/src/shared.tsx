    ...
    <Popover open={open} onOpenChange={o => { setOpen(o); if (o) { setDraft(value); setPreset(null) } }}>
      <div className="flex shrink-0 gap-1">
        <PopoverTrigger asChild>
          <Button variant="outline" size="sm" className="date-range-control h-9"><CalendarDays data-icon="inline-start" /><span className="tabular-nums">{label}</span></Button>
        </PopoverTrigger>
        {value?.from && (
          <Button variant="outline" size="icon-sm" aria-label="기간 초기화" onClick={() => onChange(undefined)}><X /></Button>
        )}
      </div>
