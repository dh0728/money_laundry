...
import { ArrowRight, Check, CircleDollarSign, ExternalLink, FileClock, History, Link2, Radar, Save, UserRound } from 'lucide-react'
  ...
  return (
    <div className="min-h-full flex flex-col gap-5">
            ...
            <Card className="shadow-none"><CardContent>
              ...
              <ChartContainer config={{ USD: { label: 'USD', color: 'var(--muted-foreground)' } }} className="h-[170px] w-full">
                <BarChart data={daily} margin={{ left: 0, right: 4, top: 6 }}>
                  ...
                  <ChartTooltip cursor={{ fill: 'var(--muted)', opacity: .35 }} content={<ChartTooltipContent formatter={v => `USD ${usd(Number(v))}`} hideIndicator />} />
      ...
      {tab === 'transactions' && (() => {
        ...
        const senderOwner = picked ? transactionIndex.accounts.find(item => item.id === picked.from)?.owner ?? picked.from : undefined
        return (
          <div className="grid gap-4 @5xl:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)] items-start">
            ...
            <Card className="selected-transaction-card shadow-none"><CardContent className="space-y-6">
              ...
              {picked && <dl className="grid grid-cols-2 gap-5 text-xs">
                <div><dt className="text-muted-foreground">거래 ID</dt><dd className="mt-1 flex items-center gap-1 font-mono">{picked.id}{onOpenTransaction && <Button type="button" variant="ghost" size="icon" className="size-7" aria-label="Transactions에서 거래 ID 보기" title="Transactions에서 거래 ID 보기" onClick={() => onOpenTransaction({ type: 'transaction', transactionId: picked.id })}><ExternalLink className="size-3.5" /></Button>}</dd></div><div><dt className="text-muted-foreground">일시</dt><dd className="mt-2 tabular-nums">{picked.at}</dd></div>
      ...
      {tab === 'conclusion' && (
        <div className="grid flex-1 gap-4 @5xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,1fr)] items-start">
          {/* Figma v17/v18: textarea min-h + flex fill; v18 R2는 resize-y로 크기 조절 핸들 제공 */}
          <Card className="shadow-none h-full"><CardContent className="flex flex-col gap-6 h-full">
