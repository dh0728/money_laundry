...
export type RecordItem = {
  ...
  redetection?: {
    priorAlertId: string; priorClosedAt: string; priorOutcome: string; account: string
    newTransactionId: string; newTransactionAt: string; detectedAt: string
  }
}
...
// 금액(USD) → 선 굵기. 패널에 보이는 엣지들의 min~max usd를 로그 척도로 1~12px에 매핑한다(v17: 그라디언트처럼
// 보이던 제곱근 폭 대신 균일한 선 굵기 자체가 로그 스텝마다 뚜렷이 달라지게). min===max면 중간값(3px).
...
const titles: Record<Pattern, string> = {
  ...
  BIPARTITE: '두 계좌 집단 간 교차 이체', STACK: '중계 계좌를 겹겹이 경유한 이동', RANDOM: '불규칙 경로의 연쇄 이체', NON_PATTERN: '패턴 외 의심 거래 묶음',
}
...
// Frontend 제안 mock: 같은 STACK 블록(같은 계좌)을 공유하는 과거 종결 Alert와 현재 재탐지 Alert를 연결한다.
// backend 재탐지 정책/API가 확정됐다는 뜻이 아니며, 과거 Alert의 종결 상태는 그대로 둔다.
const priorClosedAlert = alerts[6], reDetectedAlert = alerts[15]
const reDetectedModel = graphFor(reDetectedAlert, alerts)
reDetectedAlert.date = '2026-09-13'; reDetectedAlert.age = 3; reDetectedAlert.status = '검토 중'
reDetectedAlert.redetection = {
  priorAlertId: priorClosedAlert.id,
  priorClosedAt: '2026-09-10 16:20',
  priorOutcome: '정상 거래로 종결',
  account: reDetectedModel.nodes.find(node => node.core)?.account ?? reDetectedModel.nodes[0]?.account ?? 'UNKNOWN',
  newTransactionId: 'TX-REDETECT-2026-0913-01',
  newTransactionAt: '2026-09-13 09:42',
  detectedAt: '2026-09-13 09:47',
}
