...
describe('Figma v14 수정사항 · 관계 그래프', () => {
  ...
  it('화면 맞춤은 상단 전체화면이 아니라 하단 확대·축소 묶음에 있다', () => {
    expect(markup.indexOf('aria-label="화면 맞춤"')).toBeGreaterThan(-1)
    expect(markup.indexOf('aria-label="화면 맞춤"')).toBeGreaterThan(markup.indexOf('aria-label="확대"'))
  })
...
describe('Figma v14 수정사항 · 화면별', () => {
  ...
  it('Dashboard 개인: 업무마다 독립 카드, 전체 목록 button 없음', () => {
    ...
    expect(readFileSync(new URL('./index.css', import.meta.url), 'utf8')).toMatch(/\.work-card[\s\S]*?border-color:/)
  })
...
describe('RDR 9000 조사 도우미', () => {
  it('닫혀 있을 때 Notion-style FAB(렌즈 아이콘)로 열고, 열리면 FAB는 숨긴다', async () => {
    ...
    expect(closed).toContain('RDR 9000') // aria-label
