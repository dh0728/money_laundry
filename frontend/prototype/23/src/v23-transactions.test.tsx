...
describe('v23 Transactions four-stage explorer', () => {
  ...
  it('limits internal scrolling to owners and stacks detail below at narrow desktop widths', () => {
    ...
    expect(css).toMatch(/@media\s*\(max-width:\s*1100px\)[\s\S]*\.transactions-flow\s*\{[^}]*grid-template-columns:\s*minmax\(12rem,\.8fr\) minmax\(13rem,\.9fr\) minmax\(16rem,1\.2fr\)[^}]*\}[\s\S]*\.transaction-detail\s*\{[^}]*grid-column:\s*1\s*\/\s*-1/s)
