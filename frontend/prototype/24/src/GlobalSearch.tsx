    ...
    <Popover open={open} onOpenChange={o => { setOpen(o); if (!o) setActive(0) }}>
      <PopoverAnchor asChild>
        <div ref={searchAnchorRef} className="relative">
          ...
          />
          <Kbd data-testid="global-search-shortcut" aria-hidden="true" className="absolute right-2 top-2 z-10 pointer-events-none">/</Kbd>
        </div>
      </PopoverAnchor>
      <PopoverContent id="global-search-listbox" role="listbox" aria-label="검색 결과" className="w-(--radix-popper-anchor-width) max-w-[90vw] p-2" onOpenAutoFocus={e => e.preventDefault()} onInteractOutside={event => {
        if (searchAnchorRef.current?.contains(event.target as Node)) event.preventDefault()
      }}>
        {flat.length === 0
