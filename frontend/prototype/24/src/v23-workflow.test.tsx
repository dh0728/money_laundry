...
describe('v23 Alert episode linking workflow', () => {
  ...
  it('shows the mode entry only on Alert lists and keeps checkboxes out of browse mode', () => {
    ...
    const trigger = alerts.slice(alerts.lastIndexOf('<button', alerts.indexOf('Episode로 묶기')), alerts.indexOf('>', alerts.indexOf('Episode로 묶기')) + 1)
    expect(trigger).toContain('data-variant="outline"')
    expect(alerts).toMatch(/lucide-(combine|link-2)/)
