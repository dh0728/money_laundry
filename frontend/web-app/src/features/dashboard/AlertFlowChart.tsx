// v24 blocks/chart-area-interactive.tsx. 기간은 대시보드의 DateRangeButton이 정하고 여기선 받은 데이터만 그린다.
...
const chartConfig = {
  ...
  closed: { label: '종결', color: dashboardChartTones.secondary },
...
export function AlertFlowLegend() {
  return <div className="flex items-center justify-center gap-5 pt-3 text-xs text-muted-foreground">
    ...
    <span className="inline-flex items-center gap-2"><i data-line-style="solid" className="w-5 border-t-2" style={{ borderColor: dashboardChartTones.secondary }} aria-hidden />종결</span>
  </div>
}
...
export function ChartAreaInteractive({ data }: { data: DailyFlow[] }) {
  return (
    <Card className="@container/card min-w-0 max-w-full" data-testid="alert-flow-chart">
      <CardHeader>
        <CardTitle>일별 Alert 유입·종결</CardTitle>
        <CardDescription>
          <span className="hidden @[540px]/card:block">새로 생긴 Alert와 종결된 Alert 추이</span>
          <span className="@[540px]/card:hidden">유입·종결 추이</span>
        </CardDescription>
      </CardHeader>
      <CardContent className="min-w-0 px-2 pt-4 sm:px-6 sm:pt-6">
        <ChartContainer config={chartConfig} className="aspect-auto h-[250px] min-w-0 max-w-full w-full">
          <AreaChart accessibilityLayer data={data}>
            ...
            <Area dataKey="closed" name="종결" type="natural" fill="url(#fillClosed)" stroke="var(--color-closed)" strokeWidth={2.5} activeDot={false} />
