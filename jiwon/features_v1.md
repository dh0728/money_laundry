...
대상: HI-Small, `ts < 2022-09-11` (꼬리 제외, 팀 합의). 총 61개 = 단건 7 + 계좌 속성 2 + 그래프 52.
...
| 피처 | 정의 |
|---|---|
| from_entity_type / to_entity_type | accounts 파일 `Entity Name`에서 `" #숫자"` 태그 제거 후 유형 (범주 6: Partnership, Corporation, Sole Proprietorship, Country, Individual, Direct) |

조인 키 = (은행, 계좌번호). 거래 파일의 은행 코드는 앞자리 0 제거 후 accounts의 Bank ID와 매칭 (매칭률 100%). 정적 등록 정보라 시점 누수 없음.
